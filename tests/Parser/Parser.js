(function(QUnit, GrapeQLCoach) {
    ...
    function testGetDbColumn(assert, test) {
        ...
        // getDbColumn based on .findParent
        select.addChild(link);
        
        let dbColumn = select.getDbColumn({
            server: test.server
        }, link);
        
        assert.equal(dbColumn, test.column, `find ${ test.link } in
             ${ test.node }`);
    }
    ...
    QUnit.test("Selec.getDbColumn", function(assert) {
        ...
        testGetDbColumn(assert, {
            ...
            link: "company.id",
            column: SERVER_1.schemes.public.tables.company.columns.id
        });
        ...
        testGetDbColumn(assert, {
            ...
            link: "country.id",
            column: SERVER_1.schemes.public.tables.country.columns.id
        });
        ...
        testGetDbColumn(assert, {
            ...
            column: SERVER_1.schemes.test.tables.company.columns.id
        });
    });
